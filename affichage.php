...
$req = "select * from  enregistrement1  where  (date<='$datef' and date>='$dated') and iduser='$id' and ids='$ids' order by date ";
$res = mysqli_query($connect, $req);
	...
	echo '
		...
		<body>
			...
			<script>
					...
					swal({
						title: "Pas D Enregistrement Ou Date Deja Saisie ??? ",
						icon: "warning",
					})
					.then((ok) => {
...
while ($t = mysqli_fetch_array($res)) {
	echo '
	  <tr>
		...
		<td width="100"  align="center"><button type="submit" style="font-size: 14pt; color: #570f0f" name="supp"  value="' . $t["id"] . '">Supprimer</button></td>
			</form>
		  </tr>';
	$nbt = $nbt + 1;
	$nbq = $nbq + $t["qunatite"];
